...
from streaming.helpers import (
    format_link_type,
    generate_stream_badges,
    generate_stream_description,
    get_stream_filename,
    infer_format_from_url,
    normalize_codec,
)
...
async def handle_stream(request: Request, type: str, id: str, config_str: str):
    ...
    for stream_data in stream_results:
        ...
        # --- Metadata enrichment (surfaced for all three layouts) ---
        # Codec: v3 carries codec_name on the model; v1/v2 don't expose it.
        codec = normalize_codec(stream_data.get("codec_name"))
        # Format: prefer v3 linkType, fall back to URL extension inference.
        fmt = format_link_type(stream_data.get("link_type")) or infer_format_from_url(url_str)

        # Title line: movie title + optional [Language] tag.
        item = stream_data.get("match_item")
        item_title = getattr(item, "title", None) or getattr(item, "name", None) or title
        if audio_lang and audio_lang.lower() not in (item_title or "").lower():
            display_title = f"{item_title} [{audio_lang}]"
        else:
            display_title = item_title

        # One renderer per layout option in the addon's UI selector.
        ...
        # Native Stremio chips — these render as colored pills above the
        # stream title in the picker UI, mirroring PenguPlay's second line.
        badges = generate_stream_badges(resolution, codec, audio_langs_display, fmt=fmt)
