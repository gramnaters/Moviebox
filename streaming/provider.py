...
from streaming.proxy_rotator import get_proxy_for_request, mark_proxy_failed, mark_proxy_success, get_proxy_status

# Backward-compatible single PROXY_URL (first one)
PROXY_URL = os.getenv("PROXY_URL", "").split(",")[0].strip() if os.getenv("PROXY_URL", "") else ""
...
async def search_v2(title: str, year: str, is_movie: bool):
    ...
    try:
        s = SessionV2(proxy=proxy or None)
...
async def search_v1(title: str, year: str, is_movie: bool):
    ...
    proxy = get_proxy_for_request()
    try:
        s = SessionV1(proxy=proxy or None)
...
async def search_v3(title: str, year: str, is_movie: bool):
    matches = []
    try:
        s = SessionV3(proxy=PROXY_URL or None)
        await s.start()
            ...
            all_streams.append(
                {
                    ...
                    # Surfaced for PenguPlay-style metadata (v3 has both;
                    # v1/v2 fall back to None and get inferred from URL later).
                    "codec_name": getattr(dl, "codec_name", None)
                    or getattr(dl, "codecName", None),
                    "link_type": getattr(dl, "link_type", None)
                    or getattr(dl, "linkType", None),
